...
var Btc = function () {
  function Btc(transport) {
    ...
    transport.decorateAppAPIMethods(this, ["getWalletPublicKey", "signP2SHTransaction", "signMessageNew", "createPaymentTransactionNew"], "SKY");
  }
  ...
  (0, _createClass3.default)(Btc, [{
    ...
    value: function getWalletPublicKey_private(path, verify, segwit) {
      ...
      return this.transport.send(0xe0, 0x40, p1, p2, buffer).then(function (response) {
        ...
        var skycoinAddress = response.slice(1 + publicKeyLength + 1, 1 + publicKeyLength + 1 + addressLength).toString("ascii");
        var chainCode = response.slice(1 + publicKeyLength + 1 + addressLength, 1 + publicKeyLength + 1 + addressLength + 32).toString("hex");
        return { publicKey: publicKey, skycoinAddress: skycoinAddress, chainCode: chainCode };
      });
    }
